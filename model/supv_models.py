...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    conv_exp = Conv2dSamePadding(inp, hidden_dim, 1, 1, bias=False)
    bn1 = nn.BatchNorm2d(hidden_dim)  
    relu = nn.ReLU6(inplace=True)
    attn = relu(bn1(conv_exp(attn)))
    bn3 = nn.BatchNorm2d(oup)
    conv_pro = Conv2dSamePadding(hidden_dim, oup, 1, 1, bias=False)
    attn = bn3(conv_pro(attn))
    attn = attn.type(torch.cuda.DoubleTensor) 
    attn = attn.view(bsz * num_heads, tgt_len, src_len)
    attn_output_weights = attn+attn_output_weights
...
class Encoder(Module):
    ...
    def __init__(self, encoder_layer,encoder_layer1, num_layers, norm=None):
        ...
        self.layers1 = _get_clones(encoder_layer1, num_layers)
...
class EncoderLayer1(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        self.self_attn = MultiHead(d_model, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = Linear(d_model, dim_feedforward)
        self.dropout = Dropout(dropout)
        self.linear2 = Linear(dim_feedforward, d_model)
...
class Audio_Visual_Fusion_Attention(nn.Module):
    def __init__(self):
        super(Audio_Visual_Fusion_Attention, self).__init__()
        ...
        self.Linear_K = nn.Linear(512, 2560)
        self.Linear_Q1 = nn.Linear(128, 2560)
        self.Linear_Q2 = nn.Linear(128, 2560)
        self.Linear_V = nn.Linear(512, 2560)
    ...
    def forward(self, video, audio):
        ...
        k=self.Linear_K(visual_feature)
        q1=self.Linear_Q1(audio_feature)
        q2=self.Linear_Q2(audio_feature)
        v=self.Linear_V(visual_feature)
        
           
        kq1_t = []
        v_f1 = torch.chunk(k, spatial, 1)
        for item in v_f1:
            audio_visual_1 = self.MFB(torch.squeeze(item),q1)
            kq1_t.append(torch.unsqueeze(audio_visual_1, 1))            
        kq1 = torch.cat(kq1_t, 1)        
        
        kq2_t = []
        v_f2 = torch.chunk(v, spatial, 1)
        for item in v_f2:
            audio_visual_2 = self.MFB(torch.squeeze(item),q2)
            kq2_t.append(torch.unsqueeze(audio_visual_2, 1))            
        kq2 = torch.cat(kq2_t, 1)     

        audio_video_query = self.relu(self.affine_bottleneck(kq1))
        audio_video_query_raw = audio_video_query.mean(-2)
        channel_att_maps = self.affine_v_c_att(audio_video_query_raw).sigmoid().unsqueeze(-2)
        c_att_visual_feat = (kq2 * (channel_att_maps + 1))
        
        spatial_att_maps = self.softmax(self.tanh(self.affine_v_s_att(audio_video_query)).transpose(2, 1))     
...
class Visual_Attention(nn.Module):
    ...
    def forward(self, video, audio):
        ...
        audio_video_query_raw = video_query_1.mean(-2)
